...
export type ExpensesVsIncomeDto = {
...
export const RecordService = {
  ...
  async getExpensesVsIncome(): Promise<ExpensesVsIncomeDto> {
    const result = await db
      .select({
        year: sql<number>`EXTRACT(YEAR FROM ${records.date})`,
        month: sql<number>`EXTRACT(MONTH FROM ${records.date})`,
        totalIncome: sql<number>`SUM(CASE WHEN ${records.isExpense} = false THEN ${records.value} ELSE 0 END)`,
        totalExpenses: sql<number>`SUM(CASE WHEN ${records.isExpense} = true THEN ${records.value} ELSE 0 END)`,
      })
      .from(records)
      .groupBy(sql`EXTRACT(YEAR FROM ${records.date}), EXTRACT(MONTH FROM ${records.date})`)
      .orderBy(sql`EXTRACT(YEAR FROM ${records.date}) DESC, EXTRACT(MONTH FROM ${records.date}) DESC`);

    const monthlyData: MonthlyIncomeExpenseDto[] = result.map(row => ({
      year: row.year,
      month: row.month,
